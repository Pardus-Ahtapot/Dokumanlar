...
# Cyphon Uyarı Oluşturulması (Rsyslog)
Bu dökümanda Cyphon'a örnek bir log kaynağının rsyslog ile nasıl entegre edileceği anlatışmıştır.
Örnek olarak **suricata** uygulamasının **fastlog** tipindeki log kaynağı kullanılacaktır.

>**Not:** Cyphon'ın ansible ile kurulumunun başarılı bir şekilde yapıldığı varsayılmaktadır. Ansible ile cyphon kurulumu için [bakınız.](http://github.com/cyphon_install.md)

## Temel tasarım
**Log Gönderme**
```mermaid
graph LR
A[SuricataFastLog]-- Dahil Et --> B((Local Rsyslog))
B -- Relp veya UDP veya TCP --> C((Remote Rsyslog))
C -- Json olarak gönder --> D[Logstash]
D -- Anlamlandır ve Gönder --> E{ElasticSearch}
D -- Anlamlandır ve Gönder --> F{RabbitMQServer}
```
Yukarıdaki şemada görüldüğü üzere ;
1- Log dosyası Suricata üzerindeki rsyslog'a dahil edilir ve istenilen yöntemle Logstash'in kurulu olduğu cihazdaki Rsyslog'a gönderilir. (Eğer relp kullanılmıyor ise loglar buradan json olarak doğrudan Logstash'e gönderilebilir.)
...
```
template(name="logstash-json-template"  
type="list") {  
constant(value="{")  
...
filter {  
	...
	grok {  
		match => { "message" => "(?<ts>.*?\d+.*?\d+.*?\d+-\d+:\d+:\d+).*?\[(?<action>.*?)\]\s+\[(?<generator_id>.*?):.*?(?<signature_id>.*?):.*?\]\s(?<signature>.*?)\s\[Classification:\s(?<classification>.*?)\]\s\[Priority:\s(?<priority>.*?)\].*?{(?<protocol>.*?)\}\s(?<source_ip>.*?):(?<source_port>.*?)\s-\>\s(?<destination_ip>.*?):(?<destination_port>.*)"}  
		}  
...
```
(?<ts>.*?\d+.*?\d+.*?\d+-\d+:\d+:\d+).*?\[(?<action>.*?)\]\s+\[(?<generator_id>.*?):.*?(?<signature_id>.*?):.*?\]\s(?<signature>.*?)\s\[Classification:\s(?<classification>.*?)\]\s\[Priority:\s(?<priority>.*?)\].*?{(?<protocol>.*?)\}\s(?<source_ip>.*?):(?<source_port>.*?)\s-\>\s(?<destination_ip>.*?):(?<destination_port>.*)
```
...
```
**Elasticsearch** alanında **hosts** ile elasticsearch sunucusunu tanımlanıp **index** ile kayıtların günlük olarak saklanmasını sağlayacak sınıflandırma ismi belirlenir. Daha önce oluşturduğumuz @uuid'de eklenerek elasticsearch'e gönderilir.
...
```
{
  "@timestamp": "2018-11-30T11:33:41.000Z",
  "@version": "1",
  "sysloghost": "suricata",
  "severity": "notice",
  "facility": "local0",
  ...
  "destination_port": "53",
  "@uuid": "33530cb8-7d8b-4aa8-863f-3de9694494c7",
...
**Add watchdog** denir ve aşağıdaki ayarlar yapılır.
**Name:** suricatafastlog
**Enabled** işaretlenir.
